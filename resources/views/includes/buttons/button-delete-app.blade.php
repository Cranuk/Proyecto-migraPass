<button type="button" wire:click.stop="confirmDelete({{ $appId }})" wire:loading.attr="disabled" wire:target="confirmDelete" class="button cancel small {{ $confirmingDeletion === $appId ? 'delete' : '' }}" title="Eliminar">

    <span wire:loading.remove wire:target="confirmDelete">
        @if($confirmingDeletion === $appId)
