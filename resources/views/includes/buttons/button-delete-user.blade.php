<button type="button" wire:click.stop="confirmDelete({{ $userId }})" wire:loading.attr="disabled" wire:target="confirmDelete({{ $userId }})" class="button cancel small {{ $confirmingDeletion === $userId ? 'delete' : '' }}" title="Eliminar">

    <span wire:loading.remove wire:target="confirmDelete({{ $userId }})">
        @if($confirmingDeletion === $userId)
    ...
    <span wire:loading wire:target="confirmDelete({{ $userId }})">
